...
class Deployer {
  ...
  constructor(account: Signer) {
    ...
    this.gasPrice = "";
    this.balance = "";
    this.lzEndpoint = ethereumEndpointAddress; //TODO: change to Ethereum mainnet
    this.token = null;
    this.vesting = null;
  }
  ...
  async initialize(): Promise<void> {
    ...
    console.log(
      `\nEthereum Deployer Initialized. \nAccount: ${this.accountAddress} \nBalance: ${this.balance} \nGas Price: ${this.gasPrice} Gwei`
    );
  }

  async deployToken(): Promise<void> {
    console.log("\nDeploying Stablecomp token to Ethereum...");
  ...
  async verifyContract(): Promise<void> {
    ...
    console.log("\nVerifying Stablecomp token on Ethereum...");
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log("\n=== Ethereum Token Deployment Completed Successfully ===");
  } catch (error: any) {
    console.error(`\n!!! Error in Ethereum Token Deployment:`, error.message);
    process.exitCode = 1;
  }
}
